...
const INTROS = {
  ...
  '🧭-인프라-개요': {
    what: 'Track A 의 출발점입니다. 인프라와 클라우드가 무엇인지, 가상화가 왜 필요한지 다룹니다.',
    how: [
      '용어가 낯설어도 괜찮습니다. 여기서 물어보고 넘어가면 됩니다.',
      '이 서버 자체가 실물 예제입니다 — https://ktci5.kr/study/infra 의 **L1 기초**부터 보시면 감이 잡힙니다.',
      '개념이 잡히면 아래 채널들이 각각 어느 부분을 다루는지 보입니다.',
    ],
  },
  '💻-리눅스': study('리눅스'),
  '🌐-네트워크': study('TCP/IP, 라우팅, 방화벽 등 네트워크'),
  '☁️-클라우드': study('KT클라우드와 가상화, IaC'),
  '🐳-도커-컨테이너': study('도커 컨테이너 실무와 아키텍처'),
  ...
  '📚-자료공유': {
    what: '강의 자료, 정리 노트, 실습 스크립트를 모으는 곳입니다.',
    how: [
      '**교육과정 드라이브 폴더** 링크는 이 채널의 다른 고정 메시지에 있습니다.',
      '깃허브에 올린 정리 문서는 `/스터디자료` 로 디스코드에서 바로 볼 수 있습니다.',
      '직접 만든 파일은 `/자료함` 으로 채널 폴더를 열어 올려주세요.',
      '어떤 파일이 있는지는 `/자료보기`, 찾을 때는 `/자료검색`.',
      '파일 이름에 `#태그` 를 넣어두면 검색이 쉬워집니다.',
    ],
  },
  '🔗-유용한링크': {
  ...
  '🎯-기본프로젝트': {
    what: '1차(기본) 프로젝트를 준비하는 곳입니다. Track A 에서 배운 것을 하나로 잇는 단계입니다.',
    how: [
      '예상 파이프라인: **Python 웹앱 → Docker → Kubernetes → AWS → Terraform**',
      '각 단계에서 막힌 것은 해당 분야 채널(#🐳-컨테이너-쿠버네티스, #☁️-클라우드)에서 먼저 풀고, 여기엔 **결정과 진행 상황**만 남깁니다.',
      '무엇을 만들지 정하는 것부터 시작합니다. 기능보다 **어떤 인프라 구성을 보여줄지**가 평가 대상일 가능성이 높습니다.',
      '일정은 `/일정등록` 으로 캘린더에 올려 마감을 눈에 보이게 둡니다.',
    ],
    example: '[9/10] 앱 정하기 — 간단한 방명록 API 로 갈지, 파일 업로드 서비스로 갈지\n기준: DB 연결과 스토리지를 둘 다 보여줄 수 있는가',
  },
  '🐍-python': {
    what: 'Track A 의 Python 파트이자, 프로젝트에 올릴 애플리케이션을 만드는 곳입니다.',
    how: [
      '화려할 필요 없습니다. **컨테이너로 말아서 K8s 에 올릴 수 있으면** 충분합니다.',
      'Flask 나 FastAPI 로 엔드포인트 두세 개면 시작됩니다. 헬스체크(`/health`)는 꼭 넣으세요 — K8s 가 씁니다.',
      '환경변수로 설정을 받게 만들어 두면 Docker·K8s 로 넘어갈 때 고생이 줄어듭니다.',
      '코드는 `/자료함` 으로 폴더에 두거나 깃허브에 올립니다.',
    ],
  },
  '🏗️-terraform-iac': {
    what: '인프라를 손으로 만들지 않고 코드로 정의하는 곳입니다.',
    how: [
      '콘솔에서 클릭으로 만든 자원은 **재현이 안 됩니다.** 같은 걸 다시 만들라면 기억에 의존해야 합니다.',
      'Terraform 은 원하는 상태를 파일에 적어두면 그대로 맞춰줍니다. `plan` 으로 먼저 확인하고 `apply` 로 반영합니다.',
      '`terraform.tfstate` 에 실제 자원 정보가 들어갑니다. **깃허브에 올리지 마세요** — 민감 정보가 들어 있습니다.',
      '이 서버의 `wrangler.toml` 도 같은 발상입니다. https://ktci5.kr/study/infra 의 L4 참고.',
    ],
  },
  '📐-설계노트': {
    what: '왜 그렇게 정했는지 남기는 곳입니다.',
    how: [
      '**고민한 흔적이 곧 발표 자료가 됩니다.** 결과만 있으면 "왜 이렇게 했나요" 에 답하기 어렵습니다.',
      '선택지를 두세 개 적고, 무엇을 기준으로 골랐는지 한 줄이면 충분합니다.',
      '나중에 되돌아볼 때 "그때 왜 이랬지" 를 막아줍니다.',
    ],
    example: 'DB: RDS vs 컨테이너 내 MySQL\n→ RDS 선택. 백업·장애조치를 직접 안 만들어도 되고, 관리형 서비스를 써봤다는 점도 남음.\n→ 다만 비용이 붙으니 프리티어 범위 확인 필요.',
  },
  '운영진': {
    what: '운영진 전용 채널입니다.',
    how: [
      '**운영 구조와 절차**는 https://ktci5.kr/study/infra 의 L4 마스터 단계에 정리되어 있습니다. 시크릿 복구, 배포 되돌리기, 상태 확인 방법이 있습니다.',
