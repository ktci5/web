...
`upload-notes.mjs` 는 **바뀐 장만 골라내고, 바뀐 게 없으면 올리지 않습니다.**

```
  신규 cond   조건문과 종료 상태   5,590자
  수정 sed    grep 확장과 sed     4,345자
    ·  awk    awk                4,046자   ← 그대로
```

내용이 바뀐 장만 `updated` 날짜가 새로 찍히고, 그 날짜가 장 아래에
"마지막 갱신"으로 표시됩니다. 강제로 올리려면 `--force`.

`--notify` 를 붙이면 관련 디스코드 채널에 알림글이 올라갑니다.
...
see: bash/loop, admin/cron        # 함께 보기 (선택)
---
...
본문에서 다른 장을 가리킬 때는 **`[[과목/장]]`** 으로 씁니다.
같은 과목이면 **`[[장]]`** 으로 줄일 수 있습니다. 링크 글자는 그 장의
제목으로 자동으로 채워지므로, 제목을 고쳐도 링크가 어긋나지 않습니다.
